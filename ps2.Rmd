...
performance_hyp_d = filtered_d %>% 
  select(-starts_with("Game"),-DoNotUseVideoGamePerformanceData) # your code here: remove the columns containing "Game" in the name
```
...
rating_hyp_long_d = rating_hyp_long_d %>%
  mutate(IsRecall = grepl("Friends",Measurement)|grepl("Strangers",Measurement))
```
...
performance_diff_d = performance_hyp_long_d %>%
  ...
  select(-c("Score", "Measurement")) %>% # now we remove columns we don't need (bonus: leave them in and see if you can understand what goes wrong!) --> because since we're spreading the data, R doesn't know which of the two possible "Score" and "Measurement" value to use for each combined row. Cond is okay because it's consistent across participants.
...
Now we're finally able to reproduce Fig. 2 from Tamir et al., we just need to get the mean differences within each game and each kind of music, and save them to a variable called `MeanImprovementScore`:
...
performance_diff_summary_d = performance_diff_d %>%
  ...
  summarise(MeanImprovementScore = mean(ImprovementScore, na.rm=TRUE), 
            lowerSEM = MeanImprovementScore - (0.5*(sd(ImprovementScore, na.rm=TRUE)/sqrt(length(ImprovementScore)))), 
            upperSEM = MeanImprovementScore + (0.5*(sd(ImprovementScore, na.rm=TRUE)/sqrt(length(ImprovementScore)))))
```
...
ggplot(performance_diff_summary_d, aes(x=ConfrontationalGame, y=MeanImprovementScore, fill=MusicCondition)) +
  geom_bar(position=dodge, stat="identity") + 
  scale_fill_brewer(palette="Set1")+
  geom_errorbar(position= dodge, aes(ymin = lowerSEM, ymax = upperSEM), width = .2)
```
